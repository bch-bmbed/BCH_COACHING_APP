    ...
    suspend fun snapshots(days: Int): JSONArray {
        ...
        for(index in (days-1) downTo 0) {
            ...
            for(s in daySessions) {
                val sessionActive=EnergyIntervals.sessionTotalForSource(s.metadata.dataOrigin.packageName,s.startTime,s.endTime,activeRecords)
