...
- **Base hors sport + activités du jour** : méthode proposée pour distinguer les journées avec et sans sport. Dépense retenue = base hors sport + activités réalisées retenues ; objectif alimentaire = max(0, dépense retenue − déficit cible). La base inclut repos, digestion et mouvements ordinaires. Les séances importées dédupliquées sont prioritaires ; sans séance importée, les séances manuelles réalisées et la marche supplémentaire chiffrée sont utilisées. Les saisies manuelles restent à part lorsqu’il existe des imports, pour éviter les copies sans horaires. Les séances prévues ne créditent pas encore le budget et les pas ordinaires ne sont pas ajoutés. Chaque calorie supplémentaire compte sans seuil. L’objectif sans sport est calculé automatiquement dans Compte. Les totaux cumulés importés restent informatifs dans ce mode ; un total quotidien saisi manuellement remplace tout le calcul.

Si la dépense quotidienne totale de la montre est renseignée, elle remplace entièrement la dépense du bilan estimé dans les trois modes. Ne pas saisir ce total comme une séance. Une donnée manquante reste inconnue ; le zéro doit être saisi explicitement. En mode base, faute de calories d’activité, la base reste affichée provisoirement avec « activité encore inconnue » ; un import vide autorisé est distinct et signifie zéro séance reçue. Si certaines séances seulement sont chiffrées, le supplément connu est utilisé et le caractère incomplet reste signalé. Les pas seuls restent informatifs. En mode maintien, une activité sans calories ne bloque pas le bilan puisque sa dépense est déjà comprise dans la moyenne. Un résultat positif est un déficit, un résultat négatif est un surplus.
...
Le bilan du mode base affiche cinq lignes : base hors sport estimée, activités supplémentaires, dépense retenue, déficit cible et objectif alimentaire. Il ne reprend pas l’ancienne enveloppe d’activité. Les modes au maintien restent disponibles pour les anciens objectifs datés ; changer de méthode n’impose aucune nouvelle valeur aux autres comptes. Aucun changement de schéma ou d’APK. Vérification : 117 tests JavaScript, dont les semaines à 4 et 5 séances, les jours de repos, l’ajout sous l’ancien seuil, le budget après réalisation, le total manuel, les imports absents ou partiels, les copies et le passage daté entre deux appareils.
...
Dans Activités, le repère de marche Urevo utilise la dernière pesée à la date de la séance, la durée importée et l’allure reçue. `walkingReference(session, weight, resting)` privilégie la distance complète divisée par la durée du même enregistrement, pauses comprises. Sans distance, la moyenne des points de vitesse sert à une comparaison explicitement conditionnelle : ces points ne garantissent pas la couverture de toute la séance. Aucune allure de 4 km/h n’est substituée aux données manquantes. La pente n’étant pas transmise, le tapis à 0 % reste une hypothèse visible.

La vitesse arrondie au dixième sélectionne une tranche de marche sur tapis entre 1,9 et 8,9 km/h dans le [Compendium 2024, entrées 17346–17367](https://pacompendium.com/walking/). Aucune extrapolation hors de ce domaine. Dépense totale théorique ≈ MET × poids × durée en heures ; repos = métabolisme du profil × durée / 24 h ; dépense active = max(0, différence). Le détail affiche la tranche, le facteur d’intensité et chaque terme du calcul. Les MET sont des repères moyens, pas une mesure individuelle ni une validation de la méthode Urevo. Les pauses, variations d’allure et inclinaisons limitent cette comparaison.
...
L’activité supposée incluse est présentée comme une hypothèse à calibrer avec le maintien et les tendances de poids, distincte du déficit cible. Elle ne se déduit ni du déficit, ni de la différence entre maintien et métabolisme basal. L’ajustement uniquement à la hausse peut surestimer la dépense si la période est moins active que celle supposée dans le maintien. Aucun chiffre nutritionnel personnel n’est imposé ou modifié par cette mise à jour.

Validation du repère basé sur les données : 106 tests JavaScript passent, couvrant l’effet de l’allure et de la durée, la priorité à la distance complète, les mesures ponctuelles, les sources dédupliquées aux horaires décalés, les limites des tranches et les données absentes ou nulles. Les calories source et le journal restent inchangés. Mise à jour du site seulement ; aucune nouvelle APK ni migration.
...
# Profil de journée et marches hors séances

Le mode base hors sport accepte un profil daté `routine` : travail assis/debout/mobile/physique, jours, deux plages horaires, intensité habituelle en MET, base des jours sans travail facultative et durée minimale de marche. Le calendrier peut être remplacé pour une date par `routineDay`. Le métier ne prescrit pas une dépense quotidienne : la base reste un réglage indépendant. Les anciennes périodes sans profil gardent leur méthode.

Le supplément d’un créneau est `max(0, calories actives retenues − activité déjà prévue sur ce créneau)`. La part prévue au-dessus du repos est `max(0, MET habituel × poids − métabolisme / 24) × durée en heures`. Hors travail, le repère est 1,3 MET ; au travail, l’intensité est modifiable. Le repos a déjà été retiré des calories actives, il ne l’est pas une seconde fois. Les marches pendant les plages de travail debout/mobile/physique sont comprises dans la base par défaut, avec correction manuelle possible. Le poids utilisé est la dernière pesée connue à la date considérée. Les valeurs Urevo gardent leur nature provisoire.

La nouvelle passerelle transmet les pas agrégés par minute de Santé Connect avec la durée maximale des enregistrements source contributeurs. Seules les données d’une précision ≤2 min permettent de détecter une marche : au moins 10 min par défaut (modifiable 5–30), au moins 40 pas/min sur les minutes retenues, pauses ≤2 min et au moins 80 % de minutes en mouvement. Ce sont des heuristiques de reconnaissance, pas des seuils physiologiques. Les petites périodes et les pas quotidiens grossiers restent inclus dans la base, sans conversion automatique.

Les minutes qui touchent une séance importée sont exclues de la marche détectée, même si les calories de cette séance sont inconnues. Les copies et les chevauchements entre séances utilisent le même arbitrage que le bilan précédent. Pour les minutes restantes, priorité aux calories actives de la même source que les pas, avec couverture complète ; sinon estimation d’après la distance et la durée, ou hypothèse explicitement affichée de marche lente à 2,8 MET si aucune distance n’est disponible. Les données de course hors plage de référence ne deviennent pas une estimation de marche. Les calories de séance entières ne sont pas réparties sur chaque minute.

`movementOverrides` mémorise le classement automatique/inclus/supplément par créneau. Le choix est synchronisé indépendamment des repas ; un conflit sur le même créneau est signalé. Les clés utilisent début/fin : si une source modifie les limites d’un créneau, le nouveau créneau revient au classement automatique. Sans horaires, les saisies manuelles restent non cumulées avec les imports pour éviter les copies. Le cache local des données Santé Connect est limité à sept jours ; l’historique privé serveur reste conservé.

La migration `routine_activity_profile` empêche les anciens navigateurs d’effacer les nouveaux champs ; recharger suffit. Une permission Pas retirée conserve les données précédentes côté serveur, mais elles ne sont pas utilisées pour la détection tant que la permission manque. `health-bridge` accepte le protocole minute optionnel et les anciens APK. L’authentification par compte et par appareil ainsi que les RLS restent inchangées.

Références : [Compendium : travail](https://pacompendium.com/occupation/), [marche](https://pacompendium.com/walking/), [FAO : dépense quotidienne](https://www.fao.org/4/y5686e/y5686e07.htm), [agrégation Santé Connect](https://developer.android.com/health-and-fitness/health-connect/aggregate-data). Validation : 130 tests JavaScript, tests SQL des protections annulés par rollback et tests Android de précision des pas et d’isolation des calories.
